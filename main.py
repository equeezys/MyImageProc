...
def createParser():
   ...
   # parser.add_argument("mirror",choices=["x","y"])
...
class MyImage:
            ...
            for j in range(0, self.cals ):
                ...
                if j in range(0,radius):
                    ...
                    if j in range(self.cals - radius,self.cals):
                        right = 0
                arrayTmp[i, j] = np.median(self.array[i - radius * top :i + radius * bot + 1:, j - radius * left :j + radius * right + 1:], (0, 1))
                if (top * bot == 0):
                    top= 1
                    bot = 1
                if (left * right == 0):
    ...
    def gauss(self,s,g):
        ...
        for i in range(0,radius*2 + 1):
            for j in range(0,radius*2 + 1):
                kernelGauss[i,j]=math.exp(-((i-radius)*(i-radius)+(j-radius)*(j-radius))/(2*s*s))/(2*math.pi*s*s)

        self.svertka(kernelGauss)
        self.gamma(g)
    ...
    def gradient(self,s):
        ...
        for i in range(0,radius*2 + 1):
            for j in range(0,radius*2 + 1):
                kernelGaussX[i,j]=(-1.0*(i-radius)/s*s)*math.exp(-((i-radius)*(i-radius)+(j-radius)*(j-radius))/(2*s*s))/(2*math.pi*s*s)
        for i in range(0,radius*2 + 1):
            for j in range(0,radius*2 + 1):
                kernelGaussY[i,j]=(-1.0*(j-radius)/s*s)*math.exp(-((i-radius)*(i-radius)+(j-radius)*(j-radius))/(2*s*s))/(2*math.pi*s*s)
        top=1
        bot =1
        left =1
        right =1
        ...
        for i in range(0, self.raws ):
            for j in range(0, self.cals ):
                ...
                if (left == 0):
                    ...
                    left = 1
                gaussX = (window * kernelGaussX).sum(axis=(0, 1))
                gaussY = (window * kernelGaussY).sum(axis=(0, 1))
                for q in range(0,3):
                    arrayTmp[i,j,q] = int(math.sqrt(gaussX[q]*gaussX[q] +gaussY[q]*gaussY[q]))
...
if namespace.command == "mirror":
    if namespace.axe == "x": image.mirrorX()
    else:
        image.mirrorY()
elif namespace.command == "sobel":
    if namespace.axe == "x": image.sobelX()
    else:
        image.sobelY()
    ...
    if namespace.direction == "cw": image.rotateCW(namespace.angle)
    else:
